...
var baseComponente = (function() {
    ...
    function inicializarAjax(base,mostrarPrimerError){
        ...
        $.ajaxPrefilter(function(options, _, jqXHR) {
            ...
            if (typeof jqXHR.done !== "function") {
                return;
            }

            jqXHR.done(function(request) {

                if(request!=undefined && request!=undefined){
                    if(request.estatus==408 || request.estatus==404 ||request.estatus==500 ){
                        if(request.responseJSON.url===undefined){
                            redireccionar(base+"/error",request.datosAdicionales);
                        }else{
                            window.location.href = request.url;
                        }

                    }
                    if(request.estatus==200 && request.tipoRespuesta=="MENSAJES"
                        && request.tipoMensaje=="ERROR"){

                        ocultarErrores();
                        var selector="";

                        $.each(request.mensajes, function( index, value ) {
